      ...
      <section className="practical section-pad" id="practical">
        ...
        <div className="practical-grid">
          <article className="practical-card">
            <header className="practical-card-head"><span>01</span><BedIcon aria-hidden="true" weight="duotone" /><em className="status">Booking pending</em></header>
            <h3>Accommodation</h3>
          ...
          <article className="practical-card">
            <header className="practical-card-head"><span>02</span><ForkKnifeIcon aria-hidden="true" weight="duotone" /><em className="status confirmed">From the programme</em></header>
            <h3>Meals</h3>
            <ul>
              {mealsByDay().map((row) => (
          ...
          <article className="practical-card">
            <header className="practical-card-head"><span>03</span><BusIcon aria-hidden="true" weight="duotone" /><em className="status confirmed">From the programme</em></header>
            <h3>Transport</h3>
            <ul>
        ...
        <div className="maps-layout">
          <div className="location-selector" role="tablist" aria-label="Workshop locations" tabIndex={-1} onKeyDown={onMapKeys}>
            {MAP_VENUES.map((location, index) => (
              <button key={location.id} type="button" role="tab" aria-selected={activeMap === index} aria-controls="map-panel" id={`map-tab-${index}`} tabIndex={activeMap === index ? 0 : -1} onClick={() => selectVenue(index)}>
                <MapPinIcon aria-hidden="true" weight={activeMap === index ? 'fill' : 'regular'} /><span><strong>{location.name}</strong><small>{location.use}</small></span><CaretRightIcon aria-hidden="true" weight="bold" />
              </button>
            ))}
          </div>
          ...
          <div className="venue-panel" id="map-panel" role="tabpanel" aria-labelledby={`map-tab-${activeMap}`}>
              ...
              {photo
                ...
                : (
                  <div className="venue-photo photo-pending">
                    <CameraIcon aria-hidden="true" weight="duotone" />
            ...
            <div className="venue-details">
                      ...
                      ? (
                        <>
                          ...
                          <button type="button" onClick={() => copy('address', address)}>
                            {copied === 'address' ? <CheckIcon aria-hidden="true" weight="bold" /> : <CopyIcon aria-hidden="true" weight="regular" />}
                ...
                <div>
                  ...
                  <dd>
                    ...
                    <button type="button" onClick={() => copy('coords', formatCoordinates(coords))}>
                      {copied === 'coords' ? <CheckIcon aria-hidden="true" weight="bold" /> : <CopyIcon aria-hidden="true" weight="regular" />}
              ...
              {venue.pending
                ? <p className="venue-pending"><WarningCircleIcon aria-hidden="true" weight="duotone" />{venue.pending}</p>
                : null}

              <div className="venue-actions">
                <a href={osmLink(coords)} target="_blank" rel="noreferrer">
                  <MapPinIcon aria-hidden="true" weight="bold" />Open map<ArrowSquareOutIcon aria-hidden="true" weight="regular" />
                </a>
                {ridable
                  ? (
                    <a href={uberLink(coords, venue.name, address)} target="_blank" rel="noreferrer">
                      <CarIcon aria-hidden="true" weight="regular" />Ride to here<ArrowSquareOutIcon aria-hidden="true" weight="regular" />
                    </a>
                  )
                  : null}
                {website
                  ? <a href={website} target="_blank" rel="noreferrer"><GlobeHemisphereWestIcon aria-hidden="true" weight="regular" />Website<ArrowSquareOutIcon aria-hidden="true" weight="regular" /></a>
                  : null}
                {phone
                  ? <a href={telHref(phone)}><PhoneIcon aria-hidden="true" weight="regular" />{phone}</a>
                  : null}
              </div>

...
export default function ProgrammePage() {
  const scheduledItems = AGENDA.reduce((total, item) => total + item.sessions.length, 0);
  const timedIntervals = AGENDA.reduce((total, item) => total + item.sessions.filter((session) => session.end).length, 0);
  ...
  return (
    <section className="agenda-section section-pad" id="agenda">
      ...
      <dl className="programme-facts">
        <div><dt>Schedule</dt><dd>{scheduledItems} items across 5 days</dd></div>
        <div><dt>Intervals</dt><dd>{timedIntervals} with confirmed duration</dd></div>
